...
def DL_tile(url, headers, channels):
...
def DL_img(lat1: float, lon1: float, lat2: float, lon2: float,
    ...
    def build_row(row_number):
        for j in range(tl_tile_x, br_tile_x + 1):
            tile = DL_tile(url.format(x=j, y=row_number, z=zoom), headers, channels)
...
def img_size(lat1: float, lon1: float, lat2: float,
